...
void MainWindow::buildUI()
{
    ...
    auto *subLabel = new QLabel("Write .prg, .d64, .g64, and .nib files to real Commodore floppy disks");
    {
    ...
    m_cbmNameLabel = new QLabel("CBM filename:");
    ...
    m_cbmNameEdit->setToolTip(
        ...
        "Not applicable for disk image files (.d64/.g64/.nib)."
    );
    formLayout->addRow(m_cbmNameLabel, m_cbmNameEdit);

    m_verifyLabel = new QLabel;   // empty label for the blank-label column
    m_verifyCheck = new QCheckBox("Verify after write (PRG only)");
    m_verifyCheck->setChecked(true);
    m_verifyCheck->setToolTip(
        "After writing a .prg file, read it back from the disk and compare\n"
        "checksums to confirm the transfer was successful.\n"
        "Not supported for disk image files (.d64/.g64/.nib)."
    );
    formLayout->addRow(m_verifyLabel, m_verifyCheck);
    ...
    m_writeBtn->setToolTip("Write the selected file to the CBM drive");
...
void MainWindow::buildMenuBar()
{
    ...
    m_openAction = fileMenu->addAction("Open Disk File…");
...
{
    updateForFileType(path);
    // Auto-fill CBM name placeholder for .prg files
    ...
    appendLog("Type: " + OpenCBMWrapper::fileTypeDescription(OpenCBMWrapper::fileTypeOf(path)));
    updateButtons();
}

void MainWindow::updateForFileType(const QString &path)
{
    using FT = OpenCBMWrapper::DiskFileType;
    FT type = OpenCBMWrapper::fileTypeOf(path);
    bool isPrg = (type == FT::PRG);

    // CBM filename and verify options only make sense for .prg files
    m_cbmNameLabel->setVisible(isPrg);
    m_cbmNameEdit->setVisible(isPrg);
    m_verifyLabel->setVisible(isPrg);
    m_verifyCheck->setVisible(isPrg);

    // Adjust write button label to reflect what will happen
    switch (type) {
        case FT::D64:
            m_writeBtn->setToolTip("Write D64 disk image to the CBM drive using d64copy");
            break;
        case FT::G64:
        case FT::NIB:
            m_writeBtn->setToolTip("Write GCR/NIB disk image to the CBM drive using nibwrite");
            break;
        default:
            m_writeBtn->setToolTip("Write the selected file to the CBM drive");
            break;
...
void MainWindow::onWriteClicked()
{
    ...
    using FT = OpenCBMWrapper::DiskFileType;
    FT type = OpenCBMWrapper::fileTypeOf(m_dropZone->currentFile());

    QString cbmName;
    bool verify = false;

    if (type == FT::PRG) {
        cbmName = m_cbmNameEdit->text().trimmed().toUpper();
        if (cbmName.isEmpty())
            cbmName = OpenCBMWrapper::deriveCBMName(m_dropZone->currentFile());
        verify = m_verifyCheck->isChecked();
    }
    // For disk images cbmName stays empty and verify stays false —
    // setWriteParams handles both cases.
...
void MainWindow::onVerifyClicked()
{
    ...
    if (type != FT::PRG) {
        appendLog("ℹ Verify is only supported for .prg files.");
        return;
    }
...
void MainWindow::onOpenFile()
{
    QString path = QFileDialog::getOpenFileName(
        this,
        "Select a Commodore disk file",
        QDir::homePath(),
        "Commodore disk files (*.prg *.d64 *.g64 *.nib *.G64 *.NIB);;"
